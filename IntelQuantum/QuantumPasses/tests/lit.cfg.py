...
config.name = 'IntelQuantumDevTools'
...
# Temporarily disabling QuantumStatsPrint while we develop a open source solution.
config.excludes = ['Inputs', 'CMakeLists.txt', 'README.txt', 'LICENSE.txt', 'QuantumStatsPrint']
...
config.substitutions.append(
    (
        ...
        "-load-pass-plugin={}/libClangQuantumExtensions.so".format(
            config.intel_quantum_libraries
        ),
    )
)
...
config.substitutions.append(
    (
        "%input_files",
        config.intel_quantum_test_inputs,
    )
)
