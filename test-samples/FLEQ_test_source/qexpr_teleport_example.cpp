...
int main() {
    ...
    // BAD
    //qexpr::eval_hold(qexpr::printQuantumLogic(qexpr::_PrepZ(q) + teleport1_join(q,a,b)));
    // GOOD
    qexpr::eval_hold(qexpr::printQuantumLogic(qexpr::_PrepZ(q) + teleport1_bind(q,a,b)));

    // Should always produce |0>, but the BAD case sometimes produces |1>
